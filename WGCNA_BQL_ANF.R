...
if (file.exists("results/ANF")){
} else {
    dir.create("results/ANF",recursive=TRUE)   
}


#Reading the raw data (rows are the sample and columns the genes)
expressiondata = as.matrix(read.csv("data/ANF_BQL_counts.csv",row.names = 1))
...
png(file = "results/ANF/sampleClustering.png",  width = 2100, height = 700,res = 120) 
par(cex = 0.6)
par(mar = c(0,4,2,0))
...
traitData <- as.data.frame(read.csv("data/ANF_BQL_metadata.csv",row.names = 1))
...
samples_to_remove <- c("BQLHLP93ANF")
...
#If you want to save this plot in a pdf file, do not comment the line below:
png(file = "results/ANF/sampleClustering_traits.png",  width = 2300, height = 1200,res = 100) 
...
png(file = "results/ANF/softthreshold.png",    width = 1200, height = 900) 
...
png(file = "results/ANF/softthreshold_peack.png",    width = 1200, height = 900) 
#Plotting the results
cex1 = 0.9
...
png("results/ANF/freescaleplot.png")
...
png( "results/ANF/MODULE_CLUSTER_silhouette.png",width = 900, height = 600)
...
dynamicMods = cutreeDynamic(dendro = geneTree, distM = dissTOM,deepSplit = 3, 
                            pamRespectsDendro = FALSE,minClusterSize = 25);
table(dynamicMods)
...
png(file = "results/ANF/MODULE_CLUSTER_COLORS.png", width = 800, height = 600);
...
png("results/ANF/MODULE_CLUSTER_COLORS_PUBLICATION.png", width = 1200, height = 1000, res = 300)

# Crear gráfico sin título, con etiquetas claras y margen para incluir la letra "A"
par(mar = c(1, 4, 4, 1))  # Márgenes (inferior, izquierda, superior, derecha)
...
if (length(METree$labels)>2){
  ...
  png(file="results/ANF/Clustering_module_eigengenes.png",heigh=1200,width=900,res = 200)
...
png(file = "results/ANF/MODULE_CLUSTER_MERGED_COLORS.png", width = 800, height = 600)
...
moduleTraitCor =round( cor(MEs, traitData, use = "p",method = "pearson"),6);
...
png("results/ANF/module_traits_correlation.png", width = 1000, height = 900,res = 100)

par(mar = c(15, 12, 5, 5),cex=1.1,cex.axis = 1.4);
...
write.csv(MEs,file="results/ANF/MEs.csv")
...
write.csv(geneModuleMembership,file="results/ANF/ME_MEMBERSHIP.csv")

write.csv(MMPvalue,file="results/ANF/ME_MEMBERSHIP_PVAL.csv")
...
write_graph(network, file = "results/ANF/network.graphml", format = "graphml")


rownames(MEs)<- rownames(traitData)


write.csv(MEs,"results/ANF/module_eigengenes.csv")

rownames(TOM)<- tax
colnames(TOM)<- tax


write.csv(TOM,"results/ANF/ADJ_MATRIX.csv")

tax_module <- cbind(tax,mergedColors)

write.csv(tax_module,"results/ANF/Tax_modules.csv")



# Cargar la matriz de similitud TOM y la clasificación en módulos
tax_module <- read.csv("results/ANF/Tax_modules.csv", stringsAsFactors = FALSE)
TOM <- as.matrix(read.csv("results/ANF/ADJ_MATRIX.csv", row.names = 1))
...
# Mostrar tabla final
write.csv(df,"results/ANF/Module_analysis.csv")
